...
import inst2 from "./InstProfle2.module.css";
...
const ViewMyCoursesAdmin = ({
  ...
  const getExchangeRate = async () => {
    const res = await axios.get(base_URL).catch((err) => console.log(err));
  ...
  useEffect(() => {
    if (fromCurrency != null && toCurrency != null) {
      const res = axios
        ...
        .then((res) => res.data)
        .then((data) => setExchanheRate(data.rates[toCurrency]));
    }
  ...
  return (
    <React.Fragment>
      <div className={inst2.ViewCourses} style={{"background-color":"white"}}>
        <a
          href={`/course/${id}/${newPrice}/${currency}/${type}/${type2}/${pricediscount}`}
          className={inst2.ref}
        >
          <img
            src="https://source.unsplash.com/600x400/?code"
            alt="card__image"
            className={inst2.courseimg}
            width="140"
          />
          <div>
            <h2>{title}</h2>
          ...
          <div>
            {/* <i className={["material-icons-outlined"].join(" ")}>
              account_circle
            </i>{" "} */}
            <p>{Instructor}</p>
          </div>
          <div className={inst2.rat}>
            <p>{ratingCurrent}</p>
          </div>
          {ratingCurrent === 0 ? (
            <div>
          ...
          {type2 !== "CorpTrainee" && newPrice === 0 && (
            <div className={inst2.priceetext}>
              <p>FREE</p>
            </div>
          )}
          {type2 !== "CorpTrainee" &&
          ...
          {type2 !== "CorpTrainee" &&
            ...
            newPrice !== pricediscount && (
              <div>
                <div className={inst2.priceetext1}>
                  <p>
                    {newPrice} {currencySelected}
                  </p>
                </div>
                <div className={inst2.priceetext11}>
                  <p>
                    {pricediscount} {currencySelected}
                  </p>
                </div>
              </div>
            )}
...
export default ViewMyCoursesAdmin;